...
def computerTurn():
    ...
    if move == 0:
    ...
    elif move == 1 and userWentToCorner():
        index = 4
    elif canCompWin()[0]:
        index = canCompWin()[1]
    elif canUserWin()[0]:
        ...
        T = buildTree()
        if move >= 1:
            index = getIndexOfBestPath(T.getHead().getChildren())
        else:
            index = random.choice(leftover_boxes)
    item = comp[0]
...
def canCompWin():
    for i in leftover_boxes:
        boxes[i][4] = comp[1]
        if checkWin(i, getCurrentGrid()):
            boxes[i][4] = -1
            return (True, i)
        else:
            boxes[i][4] = -1
...
def buildTree():
    T = Tree()
    grid = getCurrentGrid()
    n = addChildrenToNode(comp[1], grid)
    T.setHead(n)
    T.setTotalWins(n)
    return T

def getIndexOfBestPath(children):
    max = children[0].getTotalWins()
    index = children[0].getIndex()
    for child in children:
        if(child.getTotalWins() > max):
            max = child.getTotalWins()
            index = child.getIndex()
    return index

def addChildrenToNode(turn, grid):
    n = Node(grid)
    n.setWin(0)
        ...
        for j, value in enumerate(row):
            ...
            if value == -1:
                ...
                if checkWin(index, copyGrid):
                    ...
                    child = addChildrenToNode(childTurn, copyGrid)
                n.setChild(child)
                child.setWin(win) 
                child.setIndex(index)       
    return n

def userWentToCorner():
    grid = getCurrentGrid()
    for i, row in enumerate(grid):
        for j, item in enumerate(row):
            if item == user[1]:
                index = i*3 + j
                if getPositionType(index) == "corner":
                    return True
    return False


def numberOfMovesLeft(grid):
    counter = 0
    for i in grid:
        for j in i:
            if j == -1:
...
if __name__ == '__main__':
    ...
    move = 0
    grid_length = 150
    headerFont = pygame.font.SysFont(None, 24)
